...
const NAV_ITEMS = [
  { to: "/news", labelKey: "nav_content" as const },
  ...
  return (
    <nav className="sticky top-0 z-50 flex min-h-14 flex-wrap items-center gap-3 border-b border-border-default bg-surface-raised px-5" role="navigation" aria-label="Main navigation">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <button onClick={() => openPanel("cart")} className="rounded-md bg-surface-sunken px-3 py-1.5 text-xs font-semibold text-secondary transition-colors hover:bg-border-default hover:text-primary" aria-label={t("nav_cart")} title={t("nav_cart")}>🛒 {t("nav_cart")} <span className="ml-1 inline-flex min-w-[18px] items-center justify-center rounded-full bg-danger px-1.5 py-0.5 text-[0.6rem] font-bold text-white">{totalItems}</span></button>
