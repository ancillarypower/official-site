...
const EXPLORE_ITEMS = [
  { to: "/news", labelKey: "nav_news", descKey: "landing_explore_news_desc", icon: "📰" },
    ...
    <div className="animate-fade-in mx-auto max-w-4xl space-y-16">
      ...
      <section className="space-y-6 text-center">
        ...
        <div className="flex justify-center gap-3">
          ...
          <Link
            to="/news"
            className="rounded-lg border border-border-default bg-surface-raised px-5 py-2.5 text-sm font-semibold text-secondary transition-colors hover:border-accent hover:text-accent"
          >
            {t("landing_cta_news")}
          </Link>
        </div>
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {METRICS.map(({ valueKey, labelKey, icon }) => (
            <div
              key={labelKey}
              className="rounded-lg border border-border-default bg-surface-raised p-6 text-center"
            >
              <span className="text-2xl" aria-hidden="true">{icon}</span>
              <p className="mt-2 text-xl font-bold text-accent">{t(valueKey)}</p>
              <p className="mt-1 text-xs text-secondary">{t(labelKey)}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {SERVICES.map(({ icon, titleKey, descKey }) => (
            ...
            >
              ...
              <h3 className="mt-2 text-sm font-semibold">{t(titleKey)}</h3>
              <p className="mt-1 text-xs leading-relaxed text-secondary">{t(descKey)}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {EXPLORE_ITEMS.map(({ to, labelKey, descKey, icon }) => (
            <Link
              key={to}
              to={to}
              className="group rounded-lg border border-border-default bg-surface-raised p-5 transition-colors hover:border-accent"
            >
              <span className="text-2xl" aria-hidden="true">{icon}</span>
              <h3 className="mt-2 text-sm font-semibold group-hover:text-accent">{t(labelKey)}</h3>
              <p className="mt-1 text-xs text-secondary">{t(descKey)}</p>
            </Link>
          ))}
        </div>
      </section>
