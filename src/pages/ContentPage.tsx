...
export default function ContentPage() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    switch (sort) {
      case "date_desc": posts.sort((a, b) => new Date(b.date ?? 0).getTime() - new Date(a.date ?? 0).getTime()); break;
      case "date_asc": posts.sort((a, b) => new Date(a.date ?? 0).getTime() - new Date(b.date ?? 0).getTime()); break;
      case "title_asc": posts.sort((a, b) => getPostTitle(a).localeCompare(getPostTitle(b))); break;
      case "title_desc": posts.sort((a, b) => getPostTitle(b).localeCompare(getPostTitle(a))); break;
    }
    return posts;
  }, [data?.posts, filter, sort]);

  if (selectedIndex !== null && data?.posts[selectedIndex]) {
    return <ArticleView post={data.posts[selectedIndex]} onBack={() => setSelectedIndex(null)} />;
  }
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-4 flex items-baseline gap-3 border-b border-border-subtle pb-4">
        ...
        <span className="text-xs text-tertiary">{t("total_items", { n: data.totalPosts })}</span>
      </div>
      <ContentToolbar filterValue={filter} onFilterChange={setFilter} sortValue={sort} onSortChange={setSort} sortOptions={SORT_OPTIONS} />
      <PostGrid posts={filteredPosts} onSelectPost={(i) => { const post = filteredPosts[i]; if (post) setSelectedIndex(data.posts.indexOf(post)); }} />
