...
function dispatchUnhandledRejection(reason: unknown = new Error("async failure")): void {
...
describe("unhandledrejection language selection (regression #463)", () => {
  ...
  it("uses stored app language preference over browser language", async () => {
    ...
    // Use non-Error reason so handler takes the i18n path (DEV + Error = raw message)
    dispatchUnhandledRejection("some non-error rejection");
  ...
  it("falls back to browser language when no stored preference", async () => {
    ...
    // Use non-Error reason so handler takes the i18n path
    dispatchUnhandledRejection("some non-error rejection");
